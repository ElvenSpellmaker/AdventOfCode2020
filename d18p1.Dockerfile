...
RUN set -eux; \
	...
	cd /usr/src/php; \
    # Jack added, change multiplication to be the same precedence as
    # plus.
	...
	sed -i "3627c\REGISTER_LONG_CONSTANT(\"JACK_ADD_MULT_PRECEDENCE_SAME\", 1, CONST_CS | CONST_PERSISTENT);" ext/standard/basic_functions.c; \
