...
RUN set -eux; \
	...
	apt-get install -y --no-install-recommends \
		...
		libxml2-dev \
		zlib1g-dev \
		# Need bison to regenerate the parser
		bison \
	...
	cd /usr/src/php; \
    # Jack added, change plus to be higher precedence than plus.
	rm -rf Zend/zend_language_parser.output; \
	rm -rf Zend/zend_language_parser.c; \
	sed -i "74c\%left '*' '-' '.'" Zend/zend_language_parser.y; \
	sed -i "75c\%left '+' '/' '%'" Zend/zend_language_parser.y; \
	sed -n '74p' Zend/zend_language_parser.y; \
	sed -n '75p' Zend/zend_language_parser.y; \
	sed -i "3627c\REGISTER_LONG_CONSTANT(\"JACK_ADD_MULT_PRECEDENCE_ADD_HIGHER\", 1, CONST_CS | CONST_PERSISTENT);" ext/standard/basic_functions.c; \
